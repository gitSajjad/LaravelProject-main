    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                <a href="{{ route('property.value.index', $property->id) }}" class="btn btn-info btn-sm">بازگشت</a>
            </section>

            <section>
                <form action="{{ route('property.value.update', ['property' => $property->id , 'value' => $value->id] ) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <section class="row">
                        ...
                        <section class="col-12 col-md-6 my-2">
                            <div class="form-group">
                                ...
                                <select name="type" id="" class="form-control form-control-sm" id="type">
                                    <option value="0" @if (old('type', $value->status) == 0) selected @endif>ساده</option>
                                    <option value="1" @if (old('type', $value->status) == 1) selected @endif>انتخابی</option>
                                </select>
                            </div>
                            @error('type')
                                <span class="alert_required bg-danger text-white p-1 rounded" role="alert">
